...
무수이의 첫 번째 티코스 '고도'는 높은 산지에서 평지로 이어지며, 고도에 따라 달라지는 차의 향과 질감을 경험할 수 있도록 구성되었습니다.
해발 약 2000m에서 자란 맑고 깨끗한 백호은침을 시작으로, 깊고 부드러운 향을 지닌 봉황단총, 입 안에서 꽃이 피는 듯한 향을 머금은 동방미인, 그리고 평지에서 우리와 같은 공기를 마시며 비를 맞고 자란 센차까지.
산의 높이를 따라 달라지는 찻잎의 시간을 천천히 내려오며 경험해 보세요.
패키지에 포함된 코드를 입력하시면 티코스에 대한 자세한 설명을 확인하실 수 있습니다.`
