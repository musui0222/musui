...
export type MagazinePost = {
  ...
  /** Magazine author display name */
  authorDisplayName?: string | null;
};

type Props = {
  post: MagazinePost;
  /** Optional link override (e.g. archive detail /archive/xxx/0) */
  href?: string;
};

export function MagazinePostCard({ post, href }: Props) {
  ...
  const linkHref = href ?? `/magazine/${post.id}`;
