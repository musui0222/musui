...
export default function ArchiveDetailPage() {
  ...
  const archives = React.useMemo(() => getArchives(), [])
  ...
  const hasLaps = item.laps && item.laps.length > 0 && item.laps.some((s) => s > 0)
        ...
        <article className="mt-6 border border-black/12 bg-white">
          ...
          <div className="border-b border-black/10 px-4 py-3">
            <div className="flex justify-between gap-4 text-[13px]">
              ...
              <span className="font-medium text-black">{formatDate(archive.createdAt)}</span>
            </div>
          </div>
